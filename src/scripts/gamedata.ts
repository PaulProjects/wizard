export class gamedata {
  ...
  static fromJson(json): gamedata {
    ...
    if (json.dealer === undefined) {
      json = JSON.parse(json);
    }

    //validate json
    if (typeof json.dealer !== "number") {
      //backwards compatibility, some old games have a string here
      json.dealer = parseInt(json.dealer);
      if (isNaN(json.dealer)) {
    ...
    if (typeof json.round !== "number") {
      json.round = parseInt(json.round);
      if (isNaN(json.round)) {
        throw new Error("invalid dealer");
      }
    ...
    if (typeof json.max_rounds !== "number") {
      json.max_rounds = parseInt(json.max_rounds);
      if (isNaN(json.max_rounds)) {
        throw new Error("invalid dealer");
      }
    ...
    if (typeof json.score_display !== "number") {
      json.score_display = 5;
    }
    if (typeof json.time_started !== "number") {
      throw new Error("invalid time_started");
    }

    if (typeof json.time_ended !== "number" && json.time_ended !== undefined) {
      json.time_ended = undefined;
    }

    if(typeof json.id !== "string" && json.id !== undefined){
