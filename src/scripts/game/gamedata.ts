...
export class GameData implements GameState {
  ...
  constructor(config: GameState) {
    ...
    this.alt_score_change = config.alt_score_change.map(round => [...round]); // Deep copy
  ...
  static fromJSONString(jsonString: string): GameData {
    try {
      ...
      if (error instanceof SyntaxError) {
        console.error('GameData.fromJSONString - JSON Parse Error:', {
          error: error.message,
      ...
      console.error('GameData.fromJSONString - Validation Error:', {
        error: error.message,
        jsonStringLength: jsonString?.length || 0,
        fullJsonString: jsonString
      });
  ...
  static fromJson(json: any): GameData {
    try {
      ...
      console.error('GameData.fromJson - Validation Error:', {
        gameId,
        gameMetadata,
        error: error.message,
        fullGameData: json,
  ...
  private static validateAndSanitizeJson(json: any): GameState {
    ...
    const rule_random_dealer = this.validateBoolean(json.rule_random_dealer, "rule_random_dealer", false);
    const rule_expansion = this.validateBoolean(json.rule_expansion, "rule_expansion", false);
    const rule_custom_rounds = this.validateBoolean(json.rule_custom_rounds, "rule_custom_rounds", false);
    const rule_crowdchaos = this.validateBoolean(json.rule_crowdchaos, "rule_crowdchaos", false);
    const rule_altcount = this.validateBoolean(json.rule_altcount, "rule_altcount", false); // Default for backwards compatibility
    const round = this.validateNumber(json.round, "round", 1);
    const max_rounds = this.validateNumber(json.max_rounds, "max_rounds", 12);
    ...
    const score_change = this.validateArray(json.score_change, "score_change");
    const alt_score = this.validateArray(json.alt_score, "alt_score", json.score); // Backwards compatibility
    const alt_score_change = this.validateArray(json.alt_score_change, "alt_score_change", json.score_change); // Backwards compatibility
    const color = this.validateObject(json.color, "color", {}) as Record<number, RoundColor>;
    const step = this.validateNumber(json.step, "step", GameStep.PLACE_BETS); // Default for backwards compatibility
    const display = this.validateNumber(json.display, "display", GameDisplay.SCORE_OVERVIEW);
    const score_display = this.validateNumber(json.score_display, "score_display", ScoreView.ANALYTICS);
    const time_started = this.validateNumber(json.time_started, "time_started");
    const time_ended = json.time_ended !== undefined ? this.validateNumber(json.time_ended, "time_ended") : undefined;
    const id = json.id !== undefined ? this.validateString(json.id, "id") : undefined;
  ...
  private static validateNumber(value: any, field: string, defaultValue?: number): number {
  ...
  private static validateBoolean(value: any, field: string, defaultValue?: boolean): boolean {
    if (value === undefined && defaultValue !== undefined) {
      return defaultValue;
    }
  ...
  private static validateArray(value: any, field: string, defaultValue?: any[]): any[] {
    if (value === undefined && defaultValue !== undefined) {
      return defaultValue;
    }
  ...
  private static validateObject(value: any, field: string, defaultValue?: object): object {
    if (value === undefined && defaultValue !== undefined) {
      return defaultValue;
    }
  ...
  static toJsonObject(gamedata: GameData): GameState {
    return {
      ...
      alt_score_change: gamedata.alt_score_change.map(round => [...round]),
  ...
  save(): void {
    try {
      ...
      throw new StorageError("Failed to save game", { error: error.message });
    }
  }
  ...
  getCurrentScores(): number[] {
    if (this.score.length === 0) return new Array(this.players.length).fill(0);
    const lastRound = Math.min(this.round - 1, this.score.length - 1);
    return lastRound >= 0 ? [...this.score[lastRound]] : new Array(this.players.length).fill(0);
  }

  getCurrentAltScores(): number[] {
    if (this.alt_score.length === 0) return new Array(this.players.length).fill(0);
    const lastRound = Math.min(this.round - 1, this.alt_score.length - 1);
    return lastRound >= 0 ? [...this.alt_score[lastRound]] : new Array(this.players.length).fill(0);
  }

  getWinners(): string[] {
    const scores = this.rule_altcount ? this.getCurrentAltScores() : this.getCurrentScores();
  ...
  isGameFinished(): boolean {
    return this.round >= this.max_rounds && this.step === GameStep.CELEBRATION;
  }

  canAdvanceRound(): boolean {
    return this.round < this.max_rounds || this.max_rounds === Number.MAX_SAFE_INTEGER;
  }
  ...
  getDealer(): number { return this.dealer; }
  getRule_1(): boolean { return this.rule_1; }
  getRuleRandomDealer(): boolean { return this.rule_random_dealer; }
  getRuleExpansion(): boolean { return this.rule_expansion; }
  getRuleCustomRounds(): boolean { return this.rule_custom_rounds; }
  getRuleCrowdchaos(): boolean { return this.rule_crowdchaos; }
  getRuleAltcount(): boolean { return this.rule_altcount; }
  getRound(): number { return this.round; }
  getMaxRounds(): number { return this.max_rounds; }
  getPlayers(): string[] { return [...this.players]; }
  getBets(): number[][] { return this.bets.map(round => [...round]); }
  getTricks(): number[][] { return this.tricks.map(round => [...round]); }
  getScore(): number[][] { return this.score.map(round => [...round]); }
  getScoreChange(): number[][] { return this.score_change.map(round => [...round]); }
  getAltScore(): number[][] { return this.alt_score.map(round => [...round]); }
  getAltScoreChange(): number[][] { return this.alt_score_change.map(round => [...round]); }
  getColor(): Record<number, RoundColor> { return { ...this.color }; }
  getStep(): GameStep | undefined { return this.step; }
  getDisplay(): GameDisplay | undefined { return this.display; }
  getScoreDisplay(): ScoreView | undefined { return this.score_display; }
  getTimeStarted(): number { return this.time_started; }
  getTimeEnded(): number | undefined { return this.time_ended; }
  hasID(): boolean { return typeof this.id === "string"; }
  getID(): string | undefined { return this.id; }

  // Setters (keeping the original method names for compatibility)
  setDealer(dealer: number): void { this.dealer = dealer; }
  setRule_1(rule_1: boolean): void { this.rule_1 = rule_1; }
  setRuleRandomDealer(ruleRandomDealer: boolean): void { this.rule_random_dealer = ruleRandomDealer; }
  setRuleExpansion(ruleExpansion: boolean): void { this.rule_expansion = ruleExpansion; }
  setRuleCustomRounds(ruleCustomRounds: boolean): void { this.rule_custom_rounds = ruleCustomRounds; }
  setRuleCrowdchaos(ruleCrowdchaos: boolean): void { this.rule_crowdchaos = ruleCrowdchaos; }
  setRuleAltcount(ruleAltcount: boolean): void { this.rule_altcount = ruleAltcount; }
  setRound(round: number): void { this.round = round; }
  setMaxRounds(maxRounds: number): void { this.max_rounds = maxRounds; }
  setPlayers(players: string[]): void { this.players = [...players]; }
  setBets(bets: number[][]): void { this.bets = bets.map(round => [...round]); }
  setTricks(tricks: number[][]): void { this.tricks = tricks.map(round => [...round]); }
  setScore(score: number[][]): void { this.score = score.map(round => [...round]); }
  setScoreChange(scoreChange: number[][]): void { this.score_change = scoreChange.map(round => [...round]); }
  setAltScore(altScore: number[][]): void { this.alt_score = altScore.map(round => [...round]); }
  setAltScoreChange(altScoreChange: number[][]): void { this.alt_score_change = altScoreChange.map(round => [...round]); }
  setStep(step: GameStep): void { this.step = step; }
  setDisplay(display: GameDisplay): void { this.display = display; }
  setScoreDisplay(scoreDisplay: ScoreView): void { this.score_display = scoreDisplay; }
  setTimeStarted(timeStarted: number): void { this.time_started = timeStarted; }
  setTimeEnded(timeEnded: number): void { this.time_ended = timeEnded; }
