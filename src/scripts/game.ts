...
export class GameController {
	...
	private setupCallbacks(): void {
		...
		this.inputHandler.setOnTotalUpdate(
			(total: number, isValid: boolean) => {
				this.handleTotalUpdate(total, isValid);
			}
		);

		this.inputHandler.setOnInputChange(
		...
		this.colorManager.setOnColorChange(
			(round: number, color: RoundColor | null) => {
	...
	private setupRuleListeners(): void {
		...
		const rule1Checkbox = document.getElementById(
			"rule_1"
		) as HTMLInputElement;
		if (rule1Checkbox) {
	...
	private setupRuleCheckbox(
		...
		if (container && checkbox) {
			...
			checkbox.addEventListener("click", () => {
				Logger.debug("settings.checkbox.toggle", {
					id: checkboxId,
	...
	private initializeRuleStates(): void {
		...
		const classicRadio = document.getElementById(
			"calc_classic"
		) as HTMLInputElement;
		const altRadio = document.getElementById(
			"calc_alt"
		) as HTMLInputElement;
	...
	private setupModalListeners(): void {
		...
		optionBoxes.forEach((box) => {
			box.addEventListener("click", (event) => {
				...
				if (target.tagName.toLowerCase() !== "input") {
					const checkbox = box.querySelector(
						'input[type="checkbox"]'
					) as HTMLInputElement;
					if (checkbox) {
						checkbox.click();
		...
		// Settings modal open logging (if button exists inline in HTML)
		const settingsDialog = document.getElementById(
			"modal_settings"
		) as HTMLDialogElement | null;
		if (settingsDialog) {
			// Observe showModal/open state changes
			const observer = new MutationObserver(() => {
				if (settingsDialog.open) {
					Logger.event("settings.open");
				}
			});
			observer.observe(settingsDialog, {
				attributes: true,
	...
	private handleNavigationClick(): void {
		...
		if (step === GameStep.CELEBRATION) {
			...
			this.saveGame();
			this.update();
			Logger.event("tab.switch", { from: "score_overview", to: "input" });
		} else {
			this.confirmInput();
		}
	}
	...
	private handleEditDealer(): void {
		const selectDealer = document.getElementById(
			"select_dealer"
	...
	private confirmInput(): void {
		if (!this.inputHandler.confirmInput()) {
			...
			const step = this.game.getStep();
			const modalId =
				step === GameStep.PLACE_BETS ? "alert_1" : "alert_2";
	...
	private processTricks(tricks: number[]): void {
		...
		const previousClassicScores =
			currentRound > 1
				? this.game.getScore()[currentRound - 2]
				: undefined;
		const previousAltScores =
			currentRound > 1
				? this.game.getAltScore()[currentRound - 2]
				: undefined;
	...
	private updateBetDisplay(): void {
		...
		if (bets.length > 0 && bets[currentRound - 1]) {
			const totalBets = bets[currentRound - 1].reduce(
				(sum, bet) => sum + bet,
				0
			);
