...
function update() {
  if (game.getDisplay() == 1) {
    ...
    if (game.getStep() == 1) {
      ...
      document.getElementById("bet_display_container").classList.add("hidden");
...
$("#editscore").on("click", () => {
  ...
  $("td[id^='std']").on("click", function () {
    ...
    let score: number = game.getRuleAltcount()
      ? game.getAltScore()[round][player]
      : game.getScore()[round][player];
    (document.getElementById("modal_edit") as HTMLDialogElement).showModal();
    $("#edit_name").val(score);
    $("#edit_save").on("click", () => {
      $("#edit_save").off("click");
  ...
  $("td[id^='btd']").on("click", function () {
    ...
    $("#edit_save").on("click", () => {
      ...
      if (new_bet < 0 || new_bet % 1 != 0) {
        (
          document.getElementById("modal_attention") as HTMLDialogElement
        ).showModal();
...
$("#select_dealer_save").on("click", () => {
  game.setDealer(
    parseInt(
      (document.getElementById("select_dealer") as HTMLSelectElement).value
    )
  );
  if (!demomode) game.save();
  location.reload();
